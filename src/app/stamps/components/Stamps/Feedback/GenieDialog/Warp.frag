...
float yPoint(float y0,float y1,float x){
    ...
    // cos creates a smooth curve from 1 to -1, map it to 0-1
...
void main(){
    ...
    float t;
    if(uEasingFunction==0){
        t=easeOutCubic(x);
    }else if(uEasingFunction==1){
        t=easeInCubic(x);
    }else if(uEasingFunction==2){
        t=easeInOutCubic(x);
    }else if(uEasingFunction==3){
        t=easeSpring(x);
    }else{
        t=x;
    }
    
    // horizontal warp (phase: t=0.0 to t=0.4)
    ...
    // vertical warp (phase: t=0.3 to t=1.0)
    ...
    float maskY=1.-smoothstep(1.-aaWidthY,1.+aaWidthY,newUvY);
